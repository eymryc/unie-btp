...
const PROJECTS = [
  { id:1, title:"Infrastructure Routière",   cat:"Infrastructure", desc:"Réhabilitation de voirie urbaine",         color:"linear-gradient(135deg,var(--navy-light) 0%,var(--navy-mid) 100%)" },
  { id:2, title:"Complexe Résidentiel",      cat:"Bâtiment",       desc:"Immeuble R+4, 24 logements",               color:"linear-gradient(135deg,var(--navy) 0%,var(--navy-light) 100%)" },
  { id:3, title:"Pont & Ouvrage d'Art",      cat:"Génie Civil",    desc:"Ouvrage hydraulique, 80 m",               color:"linear-gradient(135deg,var(--navy-mid) 0%,var(--navy) 100%)" },
  { id:4, title:"Centre Commercial",         cat:"Bâtiment",       desc:"Surface 3 200 m², Abidjan",               color:"var(--navy)" },
  { id:5, title:"Réseau d'Assainissement",   cat:"Infrastructure", desc:"Collecteur principal, 2,5 km",             color:"linear-gradient(135deg,var(--navy-light) 0%,var(--navy-mid) 100%)" },
  { id:6, title:"Immeuble de Bureaux",       cat:"Bâtiment",       desc:"Siège social R+6, Plateau",               color:"linear-gradient(135deg,var(--navy) 0%,var(--navy-light) 100%)" },
  { id:7, title:"Aménagement Portuaire",     cat:"Génie Civil",    desc:"Extension quai industriel",               color:"linear-gradient(135deg,var(--navy-mid) 0%,var(--navy) 100%)" },
  { id:8, title:"Cité Universitaire",        cat:"Bâtiment",       desc:"480 chambres, campus moderne",            color:"var(--navy)" },
];
export default function Portfolio({ onOpenModal }: { onOpenModal: () => void }) {
  ...
  return (
    <section id="portfolio" ref={ref} className="py-[120px] relative overflow-hidden" style={{ background:"var(--navy)" }}>
          ...
          <div className="reveal flex gap-2 flex-wrap">
            {CATS.map(c => (
              <button key={c} onClick={() => setActive(c)}
                className="font-['DM_Sans'] text-[12px] font-bold tracking-[.08em] uppercase px-5 py-[10px] cursor-pointer transition-all duration-200 rounded-sm"
                style={{
        ...
        <div className="grid grid-cols-4 gap-px portfolio-grid">
          {filtered.map((p,i) => (
            <div key={p.id} className="reveal group relative overflow-hidden cursor-pointer" style={{ aspectRatio:"1", background:p.color, transitionDelay:`${(i%4)*.08}s` }}>
              ...
              <div className="absolute top-4 left-4 font-['DM_Sans'] text-[10px] font-bold tracking-[.08em] uppercase text-[--gold] bg-[color:var(--gold-pale)] border border-[rgba(237,97,32,.28)] rounded-sm px-[10px] py-1">{p.cat}</div>
              {/* Hover overlay */}
              <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-6 text-center" style={{ background:"rgba(237,97,32,.92)" }}>
                <h3 className="font-['Cormorant_Garamond'] text-[22px] font-semibold text-[--navy] mb-2">{p.title}</h3>
                <p className="font-['DM_Sans'] text-[13px] text-[rgba(15,23,42,.75)]">{p.desc}</p>
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[rgba(15,23,42,.85)] to-transparent">
                <div className="font-['DM_Sans'] text-[13px] font-semibold text-white">{p.title}</div>
              </div>
            </div>
          ))}
        </div>
        <div className="reveal text-center mt-14">
          <p className="font-['DM_Sans'] text-[14px] text-white/35 mb-6">Parcourez les réalisations des entreprises membres d&apos;UNIE-BTP</p>

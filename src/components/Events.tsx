    ...
    <section id="evenements" ref={ref} className="py-[120px] relative overflow-hidden" style={{ background:"var(--bg)" }}>
      <div className="max-w-[1280px] mx-auto px-10">
        <div className="text-center mb-20">
          ...
          <h2 className="reveal font-['Rajdhani'] font-semibold text-[--navy] leading-[1.1] mb-5" style={{ fontSize:"clamp(36px,5vw,56px)" }}>
            Nos Événements
          </h2>
        ...
        <div className="reveal rounded-sm mb-8 grid gap-[60px] items-center relative overflow-hidden featured-event"
          style={{ background:"linear-gradient(135deg,var(--night-2),var(--night-3))", padding:"60px 64px", gridTemplateColumns:"auto 1fr" }}>
          ...
          <div className="text-center bg-[--gold] p-[24px_32px] rounded-sm flex-shrink-0">
            <div className="font-['Rajdhani'] text-[11px] font-bold tracking-[.2em] uppercase text-white/80 mb-2">{EVENTS[0].month}</div>
            <div className="font-['Rajdhani'] text-[56px] font-bold text-white leading-none">{EVENTS[0].day}</div>
            <div className="font-['Rajdhani'] text-[14px] text-white/80 mt-1">{EVENTS[0].year}</div>
          </div>
          <div className="relative z-[1]">
            <div className="flex gap-2 mb-5 flex-wrap">
              <span className="font-['Rajdhani'] text-[10px] font-bold tracking-[.15em] uppercase text-[--gold] bg-[color:var(--gold-pale)] px-3 py-1 rounded-sm">Événement récent</span>
              {EVENTS[0].tags.map(t => <span key={t} className="font-['Rajdhani'] text-[10px] tracking-[.1em] uppercase text-white/40 bg-[rgba(255,255,255,.06)] px-3 py-1 rounded-sm">{t}</span>)}
            </div>
            <h3 className="font-['Rajdhani'] text-[36px] font-semibold text-white mb-5 leading-[1.2]">{EVENTS[0].title}</h3>
            <p className="font-['Rajdhani'] text-[15px] leading-[1.75] text-white/55 max-w-[560px] text-justify">{EVENTS[0].desc}</p>
          </div>
        </div>
          ...
          {EVENTS.slice(1).map((ev,i) => (
            <div key={ev.title}
              ...
              <div className="text-center min-w-[52px]">
                <div className="font-['Rajdhani'] text-[10px] font-bold tracking-[.1em] uppercase mb-1" style={{ color:ev.color }}>{ev.month}</div>
                <div className="font-['Rajdhani'] text-[36px] font-bold text-[--navy] leading-none">{ev.day}</div>
                <div className="w-6 h-[2px] mx-auto mt-2" style={{ background:ev.color }}/>
              </div>
              <div>
                <div className="flex gap-1.5 mb-3 flex-wrap">
                  {ev.tags.map(t => <span key={t} className="font-['Rajdhani'] text-[10px] tracking-[.08em] uppercase text-[#a0aec0] bg-[#f1f5f9] px-2 py-0.5 rounded-sm">{t}</span>)}
                </div>
                <h3 className="font-['Rajdhani'] text-[22px] font-semibold text-[--navy] mb-3 leading-[1.3]">{ev.title}</h3>
                <p className="font-['Rajdhani'] text-[13px] leading-[1.7] text-[#718096] text-justify">{ev.desc}</p>
              </div>
            </div>
          ))}
        </div>

...
export default function Services() {
  ...
  return (
    <section id="services" ref={ref} className="py-[120px] relative overflow-hidden" style={{ background:"var(--bg)" }}>
      <BTPBlueprint3D opacity={0.15} intensity={0.6} className="z-0" />
      <div className="max-w-[1280px] mx-auto px-10">
        <div className="text-center mb-20">
          <p className="reveal text-[11px] font-bold tracking-[.2em] uppercase text-[--gold] mb-4">Ce que nous offrons</p>
          <h2 className="reveal font-['Cormorant_Garamond'] font-semibold text-[--navy] leading-[1.1] mb-6" style={{ fontSize:"clamp(36px,5vw,56px)" }}>
            Nos axes d&apos;intervention
          </h2>
          <p className="reveal font-['DM_Sans'] text-[17px] text-[#718096] max-w-[560px] mx-auto leading-[1.7] text-justify">
        ...
        <div className="grid grid-cols-3 gap-px services-grid" style={{ background:"rgba(15,23,42,.06)" }}>
          {SERVICES.map((s,i) => (
            ...
            >
              <div className="font-['Cormorant_Garamond'] text-[72px] font-bold text-[rgba(15,23,42,.05)] leading-none absolute top-5 right-7 select-none group-hover:text-[rgba(237,97,32,.18)] transition-colors duration-350">{s.n}</div>
              ...
              <h3 className="font-['Cormorant_Garamond'] text-[24px] font-semibold text-[--navy] mb-4 leading-[1.2] group-hover:text-[--navy] transition-colors">{s.title}</h3>
              <p className="font-['DM_Sans'] text-[14px] leading-[1.75] text-[#718096] mb-6 text-justify">{s.desc}</p>
              <div className="flex flex-col gap-2">
                {s.feats.map(f => (
                  <div key={f} className="flex items-center gap-2">
                    <div className="w-1 h-1 rounded-full bg-[--gold] flex-shrink-0"/>
                    <span className="font-['DM_Sans'] text-[12px] text-[#718096]">{f}</span>
                  </div>
                ))}
              </div>

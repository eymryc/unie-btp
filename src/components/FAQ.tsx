...
export default function FAQ({ onOpenModal }: { onOpenModal: () => void }) {
  ...
  return (
    <section id="faq" ref={ref} className="py-[120px] relative overflow-hidden" style={{ background:"var(--bg)" }}>
      <BTPBlueprint3D opacity={0.12} intensity={0.5} className="z-0" />
      <div className="max-w-[1280px] mx-auto px-10">
        <div className="grid grid-cols-[1fr_2fr] gap-[100px] items-start faq-grid">
          <div className="sticky top-[120px]">
            ...
            <h2 className="reveal font-['Cormorant_Garamond'] font-semibold text-[--navy] leading-[1.1] mb-7" style={{ fontSize:"clamp(32px,4vw,48px)" }}>
              Tout ce que vous devez savoir
            </h2>
            <p className="reveal font-['DM_Sans'] text-[15px] text-[#718096] leading-[1.75] mb-10 text-justify">
            ...
            {FAQS.map((f,i) => (
              <div key={f.q} className="reveal bg-white transition-all duration-300" style={{ borderLeft: open===i ? "3px solid var(--gold)" : "3px solid transparent", transitionDelay:`${i*.06}s` }}>
                <button onClick={() => setOpen(open===i ? null : i)}
                  ...
                  <span className="font-['Cormorant_Garamond'] text-[20px] font-semibold text-[--navy] leading-[1.3]">{f.q}</span>
                ...
                {open===i && (
                  <div className="px-7 pb-7">
                    <p className="font-['DM_Sans'] text-[15px] leading-[1.8] text-[#718096] text-justify">{f.a}</p>
                  </div>
                )}
              </div>
            ))}
